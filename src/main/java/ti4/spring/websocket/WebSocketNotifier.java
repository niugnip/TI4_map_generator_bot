    ...
    public void notifyGameStateChanged(Game game) {
        try {
            ...
            synchronized (perGameLocks.computeIfAbsent(gameId, k -> new Object())) {
                ...
                if (game.isFowMode()) {
                    /**FOW views differ per-viewer, so dont stream a single patch like
                     * // non-FOW games. Refresh the (unfiltered) cache backing the GM's REST view and
                     * // tell clients to refetch web-data-fow, which re-applies each viewer's fog server-side.
                     */
